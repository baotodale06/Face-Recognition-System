...
async def init_pool() -> None:
    global _pool
    try:
        # Pool = group of workers talking to database
        _pool = await asyncpg.create_pool(DATABASE_URL, min_size=1, max_size=10)

        # auto-migrate: add image_path columsn if they do not exist (for existing DBs)
        async with _pool.acquire() as conn: # Borrow one connection from the pool
            await conn.execute(
        ...
        logger.info("Successfully initialize PostgreSQL pool")
    except Exception as e:
        logger.error(f"Fail to connect to PostgreSQL: {e}")
        
        # to we dont accidentally use a broken connection
        _pool = None
...
async def log_attendance(name: str,
                         similarity: float,
                         image_path: str | None = None) -> int | None:
    """
...
async def get_attendance(limit: int = 200, name: str | None = None,
    ...
    try:
        ...
        if date_from:
            ...
            idx += 1
        if date_to:
            conditions.append(f"detected at <= ${idx}")
...
async def get_stats():
    ...
    try:
        async with _pool.acquire() as conn:
            ...
            today_known = await conn.fetchval("SELECT COUNT(*) FROM attendance_log WHERE detected_at::date = CURRENT_DATE")
            today_unknown = await conn.fetchval("SELECT COUNT(*) FROM unknown_log WHERE detected_at::date = CURRENT_DATE")
            today_unique = await conn.fetchval("SELECT COUNT(DISTINCT name) FROM attendance_log WHERE detected_at::date = CURRENT_DATE")
...
async def save_settings(key: str, value: str) -> None:
    if not _pool_ok():
        return 
    try:
        async with _pool.acquire() as conn:
